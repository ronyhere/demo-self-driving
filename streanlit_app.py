...
def run_the_app():
    ...
    draw_image_with_boxes(image, boxes, "Real time results using YOLO",
        "**YOLO data** (frame `%i`)" % selected_frame_index)

    # Get the boxes for the objects detected by YOLO by running the YOLO model.
    yolo_boxes = yolo_v3(image, confidence_threshold, overlap_threshold)
    draw_image_with_boxes(image, yolo_boxes, "RCNN results ",
        "**RCNN % reduction by** (overlap `%3.1f`) (confidence `%3.1f`)" % (overlap_threshold, confidence_threshold))
